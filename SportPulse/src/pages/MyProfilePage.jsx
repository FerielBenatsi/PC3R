...
function DeleteConfirmModal({ username, dark, onConfirm, onCancel, loading }) {
  const [input, setInput] = useState("");
  const isValid = input === username;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-6">
      ...
      />
      <div
        className={`relative w-full max-w-sm rounded-3xl p-6 shadow-2xl z-10 ${dark ? "bg-[#0f1117] border border-white/10" : "bg-white border border-zinc-200"}`}
      >
        <div className="flex flex-col items-center gap-3 mb-6">
          ...
          <div className="text-center">
            <h2
              className={`text-lg font-black ${dark ? "text-white" : "text-zinc-900"}`}
              style={{ fontFamily: "Space Grotesk, sans-serif" }}
            >
              Supprimer le compte
            </h2>
            <p
              className={`text-sm mt-1 ${dark ? "text-zinc-400" : "text-zinc-500"}`}
            >
              Cette action est{" "}
              <span className="font-bold text-red-400">irréversible</span>. Tous
              tes pronostics et commentaires seront supprimés.
            </p>
          </div>
        </div>
        <div className="mb-5">
          <label
            className={`block text-xs font-semibold mb-2 ${dark ? "text-zinc-400" : "text-zinc-600"}`}
          >
            Tape ton pseudo{" "}
            <span className="font-black text-red-400">{username}</span> pour
            confirmer
          </label>
          <input
            type="text"
            ...
            className={`w-full text-sm rounded-xl px-4 py-2.5 outline-none transition-all ${
              dark
                ? "bg-white/5 border border-white/10 text-white placeholder-zinc-600 focus:border-red-500/50"
                : "bg-zinc-50 border border-zinc-200 text-zinc-900 placeholder-zinc-400 focus:border-red-400"
            }`}
          />
        </div>
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex-1 py-2.5 rounded-xl text-sm font-bold text-white bg-red-500 hover:bg-red-400 disabled:opacity-30 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
          >
            {loading ? (
...
export default function MyProfilePage({ dark }) {
  ...
  const pending = p.stats?.pending ?? 0;
  ...
  const inputBase = `text-sm rounded-xl px-3 py-2 outline-none transition-all duration-150 ${
    dark
      ? "bg-white/5 border border-white/10 text-white placeholder-zinc-600 focus:border-blue-500/50"
      : "bg-zinc-50 border border-zinc-200 text-zinc-900 focus:border-blue-400"
  }`;
  ...
  return (
    <>
      ...
      <main className="max-w-4xl mx-auto px-6 py-10">
        ...
        {/* actions compte */}
        <div className="mt-8 flex items-center gap-3 fade-up fade-up-6">
          <button
            onClick={logout}
          ...
          <button
            ...
            className={`flex items-center gap-2 text-sm font-semibold px-5 py-2.5 rounded-xl border transition-all ${dark ? "border-red-500/20 text-red-500/60 hover:text-red-400 hover:border-red-500/40 hover:bg-red-500/5" : "border-red-200 text-red-400 hover:text-red-500 hover:border-red-300 hover:bg-red-50"}`}
            style={{ fontFamily: "Space Grotesk, sans-serif" }}
          >
            <Trash2 size={13} />
